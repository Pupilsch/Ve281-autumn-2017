...
#ifndef INC_281_P5_BINARY_QUEUE_H
#define INC_281_P5_BINARY_QUEUE_H
#include <algorithm>
...
template<typename TYPE, typename COMP>
void binary_heap<TYPE, COMP> ::Percolate(int id) {
    while (id>1 && compare(data[id/2-1],data[id-1])){
        TYPE tmp=data[id-1];
        data[id-1]=data[id/2-1];
        data[id/2-1]=tmp;
...
#endif //INC_281_P5_BINARY_QUEUE_H
